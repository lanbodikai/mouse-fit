...
const BUDGET_TOLERANCE_PCT = 0.12;
const BUDGET_TOLERANCE_MIN_USD = 8;
...
const $status = _doc ? _doc.getElementById("status") : null;
const $grid = _doc ? _doc.getElementById("grid-grip") : null;
const $handLength = _doc ? _doc.getElementById("handLength") : null;
const $handWidth = _doc ? _doc.getElementById("handWidth") : null;
const $handSize = _doc ? _doc.getElementById("handSize") : null;
const $chosenGrip = _doc ? _doc.getElementById("chosenGrip") : null;
const $chosenGripInline = _doc ? _doc.getElementById("chosenGripInline") : null;
const $chosenGripTitle = _doc ? _doc.getElementById("chosenGripTitle") : null;
const $year = _doc ? _doc.getElementById("y") : null;
...
function filterMiceByBudget(mice, range) {
  ...
  const rawMin = toFiniteNumber(range.min);
  const rawMax = toFiniteNumber(range.max);
  if (rawMin == null || rawMax == null) return { list: mice, mode: "no-budget" };
  ...
  const tolerance = Math.max(BUDGET_TOLERANCE_MIN_USD, span * BUDGET_TOLERANCE_PCT);
  const relaxedMin = Math.max(0, min - tolerance);
  const relaxedMax = max + tolerance;

  const inRange = withPrice.filter((mouse) => mouse.price >= relaxedMin && mouse.price <= relaxedMax);
...
export function buildUserProfileFromInputs({ measurement, gripFromStorage, selections, budgetRange }) {
  const safeSelections = {
    primaryGrip: normalizeGrip(selections?.primaryGrip),
    shellShape: String(selections?.shellShape || "").trim().toLowerCase(),
    humpPosition: String(selections?.humpPosition || "").trim().toLowerCase(),
    sideShape: String(selections?.sideShape || "").trim().toLowerCase(),
    fingerDirection: normalizeFingerDirection(selections?.fingerDirection),
    thumbPosition: normalizeThumbPosition(selections?.thumbPosition),
    dominantFinger: normalizeDominantFinger(selections?.dominantFinger),
    palmFingerCurved: normalizeYesNo(selections?.palmFingerCurved),
    clawRelaxed: normalizeYesNo(selections?.clawRelaxed),
    clawBackHandTouch: normalizeYesNo(selections?.clawBackHandTouch),
  };

  const grip = safeSelections.primaryGrip || normalizeGrip(gripFromStorage) || "palm";
  const handLength = toFiniteNumber(measurement?.length_mm) ?? 180;
  const handWidth = toFiniteNumber(measurement?.width_mm) ?? 90;
  const handBucket = getHandBucket(handLength / 10);
  const dominantFinger = safeSelections.dominantFinger;
  const isSlantedHand = safeSelections.fingerDirection === "left" || safeSelections.fingerDirection === "right";
  const inwardThumb = safeSelections.thumbPosition === "inward";
  const palmSubtype = grip !== "palm" ? "" : safeSelections.palmFingerCurved === "yes" ? "palm-claw" : "full-palm";
  const clawSubtype =
    grip !== "claw"
      ? ""
      : safeSelections.clawBackHandTouch === "no"
        ? "fingertip-claw"
        : safeSelections.clawRelaxed === "yes"
          ? "relaxed-claw"
          : "regular-claw";
  ...
  return {
    ...
    clawSubtype,
    isReferenceArchetype,
    thumbPosition: safeSelections.thumbPosition,
    fingerDirection: safeSelections.fingerDirection,
    postureLengthDelta,
    postureWidthDelta,
    postureNotes,
    targetLength: personalizedSizing.targetLength,
    ...
    ratioTargetLength,
    ratioTargetWidth,
    featureRequest: buildFeatureRequest(safeSelections),
    budgetRange: budgetRange || null,
  };
}

export function runDeterministicMatcher(rawMice, profile, limit = RERANK_CANDIDATE_LIMIT) {
  const normalized = (Array.isArray(rawMice) ? rawMice : []).map(normalizeMouse);
  ...
  const localRanked = rankAndSelectTop(candidatePool, profile, retrieval, limit);
  return {
    normalized,
    cleaned,
    budgetFiltered,
...
export {
  ...
  toFiniteNumber,
  normalizeGrip,
  normalizeFingerDirection,
  normalizeThumbPosition,
  normalizeYesNo,
  normalizeDominantFinger,
  getHandSize,
  buildFeatureRequest,
  normalizeShape,
  normalizeSide,
  ...
  filterOutliers,
  filterMiceByBudget,
  retrieveByHandBucket,
  applyStrictFeatureFilters,
  expandWithRelaxedFilters,
  rankAndSelectTop,
  describeMode,
};
